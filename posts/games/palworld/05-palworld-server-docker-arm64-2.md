...
tags: [Linux, GitHub, ARM64, Issue]
---

<Postbox title="palworld-24-hours-server-bug"/>
...
services:
  palworld:
    ...
    image: thijsvanloef/palworld-server-docker:latest # [!code --]
    image: thijsvanloef/palworld-server-docker:latest-arm64 # [!code ++]
    ports:
      - 8211:8211/udp
      - 27015:27015/udp
    env_file:
...
PUID, PGID는 [클라우드로 옮기면서](/posts/palworld-server-docker-arm64/#docker-compose-yml) 변경한 것.
...
해결 방법은 환경변수를 추가하는 것으로, 최종 `docker-compose.yml` 파일은 아래와 같다.
...
services:
  palworld:
    ...
    environment:
      ...
      - UPDATE_ON_BOOT=true
      - TZ=Asia/Seoul
      - BOX64_DYNAREC_STRONGMEM=3 # [!code ++]
      - BOX64_DYNAREC_BIGBLOCK=0 # [!code ++]
      - BOX64_DYNAREC_BLEEDING_EDGE=0 # [!code ++]
    volumes:
      - ./server:/palworld
    networks:
      games:
...
FEX-Emu를 사용하지 않기 때문에 [이 문제](/posts/palworld-server-docker-arm64/#init-sh)도 없으니
이제 나도 이 이미지의 모든 기능들을 귀찮게 새로 빌드하지 않고 쉽게 사용할 수 있게 됐다..!
